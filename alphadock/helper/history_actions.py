...
class QAMenu(QMenu):
    ...
    def log_file_editor_(self, edit_id, edit_desc):
        import re
        try:
            self.log_file_ = self.parent.directory + self.parent.project_name + "/experiment_log.txt"
            ...
            new_file_ = log_file[:begin] + str_id.strip() + " " + edit_desc + log_file[end:]
    ...
    def update_history_entries(self):
        ...
        if DEBUG: print(self.log_file_labels())
    ...
    def remove_history_directory_(self, his):
        del_dir_ = self.parent.directory + self.parent.project_name + "/" + str(his)
        if not DEBUG:
            import shutil
            if DEBUG: print(del_dir_ + ":  debug_remove_his_folder")
            shutil.rmtree(del_dir_, ignore_errors=True)
        print(f"folder {del_dir_} - removed")

    def move_history_directory_(self, src, dst):
        import shutil
        src_dir_ = self.parent.directory + self.parent.project_name + "/" + str(src) + "/snapshot.pse"
        dst_dir_ = self.parent.directory + self.parent.project_name + "/" + str(dst) + "/snapshot.pse"
        if DEBUG: print(f"{src_dir_}, scr_dir")
        if DEBUG: print(f"{dst_dir_}, dst_dir_")
        shutil.move(src_dir_, dst_dir_)

    def remove_history_snapshot(self, src):
        import os
        src_dir_ = self.parent.directory + self.parent.project_name + "/" + str(src) + "/snapshot.pse"
        if DEBUG: print(f"{src_dir_}, deleting snapshot")
        if not DEBUG: os.remove(src_dir_)
    ...
    def adjust_menu_size_(self):
        ...
        try:
            if self.width() == self.maximum_menu_width:
                ...
                for act in self.children():
                    if hasattr(act, "change_middle_frame_"):
                        act.left_frame.setMaximumWidth(act.right_frame_maximum_width)
                        act.left_frame.setMinimumWidth(act.right_frame_maximum_width)
            ...
            if hasattr(act, "change_middle_frame_"):
                try:
                    ...
                    if is_snapshot_ and highlighting:
                        ...
                        act.right_btn_.setStyleSheet(QPush_action_style_snapped)
    ...
    def dropEvent(self, e):
        ...
        try:
            if DEBUG: print(f"from: {origin_} to: {drop_location_}")
            ...
            if DEBUG: print(self.snapshots_dict)
...
class QDWidgetAction(QWidgetAction):
    ...
    def initUi(self):
        ...
        self.left_btn_.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.middle_line_edit.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.middle_line_edit.setStyleSheet(line_edit_style)
        self.middle_line_edit.returnPressed.connect(self.change_experiment_log_up_)
        ...
        self.right_btn_.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def change_experiment_log_up_(self):
        ...
        self.parent.log_file_editor_(self.left_btn_.text().strip(), self.middle_line_edit.text())
    ...
    def change_right_frame_(self):
        ...
        if self.right_frame.width() == 0:
            ...
            if current_middle_frame_width > 0:
                self.middle_frame.setMinimumWidth(self.middle_frame.width() - self.right_frame.width())
                self.middle_frame.setMaximumWidth(self.middle_frame.width() - self.right_frame.width())
            ...
            if current_middle_frame_width > 0:
                self.middle_frame.setMinimumWidth(self.middle_frame.width() + self.right_frame_maximum_width)
                self.middle_frame.setMaximumWidth(self.middle_frame.width() + self.right_frame_maximum_width)
    ...
    def delete_history_item_permanently_(self):
        ...
        if DEBUG: print("delete button highlighted: ", self.is_highlighted)
